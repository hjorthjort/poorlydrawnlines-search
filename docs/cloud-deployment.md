...
This project deploys as a static Cloudflare Worker while scheduled scraping runs in GitHub Actions.
...
Make sure the Cloudflare project watches the branch that contains the committed public assets.
...
- monthly full refresh: refresh every archive page and rebuild the full search index, reusing cached OCR/page records when image URLs have not changed
...
In GitHub Actions, run **Update Poorly Drawn Lines index** manually with:
